...
export const getUserSubmodulos = cache(async (): Promise<string[]> => {
  ...
  const { data } = await supabase
    ...
    .select("submodulos(codigo), usuarios!usuario_submodulos_usuario_id_fkey!inner(activo)")
...
export async function getVistasDeModulo(modulo: string) {
  ...
  const { data } = await supabase
    .from("usuario_submodulos")
    .select("submodulos(codigo, modulo, tipo, nombre, orden), usuarios!usuario_submodulos_usuario_id_fkey!inner(activo)")
    .eq("usuario_id", user.id)
    .eq("activo", true)
    .eq("usuarios.activo", true);
