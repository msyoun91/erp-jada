...
**Verificado el 2026-09-16: PostgREST devuelve `Content-Range` en el PATCH, así que `count` llega `0` y no `null`.** Un `.update(…, { count: "exact" })` sobre un id inexistente (el mismo caso que RLS rechazando: 0 filas) devolvió `204` con `Content-Range: */0`, y supabase-js 2.112.3 lo leyó como `count: 0`. Se probó con `service_role` sobre `usuarios`, la única tabla con UPDATE para ese rol (`tareas` solo tiene GRANT para `authenticated`): el header lo pone PostgREST, no depende de la tabla ni del rol. `@supabase/ssr` usa esa misma copia de postgrest-js. Vale también para el `errorDeUpdate` de Obras.
