...
**El proyecto no se convierte en hilo: toma su cara.** Sin `estado` abierto/cerrado ni cierre automático; `ProyectoDetailPanel` muestra "X/Y terminadas" y `MetricasResumen`, cálculo puro sobre `tareasDeProyecto` (`9be9afc`). Si el proyecto tuviera estado propio, el hilo —el nivel del medio— se quedaría sin razón de existir.
