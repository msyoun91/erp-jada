-- sql/100 — transferir bloquea la fila que va a cambiar de dueño
--
-- Última entrada de la segunda pasada de la auditoría de compartir/transferir
-- (BACKLOG, 2026-09-18). Las tres funciones de transferencia leen el dueño
-- actual y después lo pisan; entre las dos cosas no había candado. Dos
-- transferencias simultáneas de la misma obra leían el mismo `v_actual`, las
-- dos escribían `responsable_id` —gana la última— y las dos armaban su rama
-- de contextuales, su log y su `resolver_vinculos` desde el mismo saliente:
-- el log queda con dos filas que dicen "de A", y los contactos migrados
-- repartidos entre dos destinos.
--
-- `FOR UPDATE` en el SELECT que lee al saliente. La segunda transacción espera
-- a la primera y reevalúa la fila ya commiteada: si el destino coincide con el
-- dueño nuevo sale por OB005, y si no, transfiere desde el dueño correcto.
-- El `AND activo` también se reevalúa, así que desactivar la obra en el medio
-- sigue dando OB004/OB025.
--
-- Sin cambios de comportamiento fuera de la concurrencia: los cuerpos son los
-- de sql/096 (obra y empresa) y sql/090 (persona), con una línea más cada uno.
--
-- `obras_migrar_agenda` no entra: no lee el dueño para después escribirlo,
-- filtra por `creado_por = p_de_usuario` dentro del propio UPDATE.
...
BEGIN
  ...
  SELECT responsable_id INTO v_actual FROM obras WHERE id = p_obra_id AND activo FOR UPDATE;
...
BEGIN
  ...
  SELECT creado_por INTO v_actual FROM obras_empresas WHERE id = p_empresa_id AND activo FOR UPDATE;
  IF v_actual IS NULL THEN
...
BEGIN
  ...
  SELECT creado_por INTO v_actual FROM obras_personas WHERE id = p_persona_id AND activo FOR UPDATE;
  IF v_actual IS NULL THEN
