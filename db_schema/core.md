...
Obras suma el suyo, que no es genérico todavía: `obras_emitir_eventos_grant('<ente>', '<columna>')` (`sql/083`) — `AFTER INSERT OR UPDATE OF activo`. Hoy cuelga de **una sola** tabla, `obras_obra_compartida`: `sql/083` lo escribió para las tres de grant directo y `sql/086` dropeó las otras dos. Las contextuales que las reemplazan no emiten a propósito — son el reflejo de la obra que sí emitió el suyo (ver `obras.md`). `activo` false→true → `compartido`, true→false → `revocado`; sin cambio de `activo`, nada (recompartir lo ya compartido reescribe `otorgada_por` y eso no movió el acceso). Ver `obras.md`.
