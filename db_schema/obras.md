...
> detector de duplicados. RLS SELECT suma `OR obras_ctx_vigente('empresa', id)` (`sql/092`; hasta ahí, `obras_empresa_grant_ctx_vigente(id)`).
...
> **MODEL A (`sql/039`, ver `decisiones/obras/visibilidad.md`).** `telefono`/`whatsapp`/`email` + sus `_norm` salieron del `GRANT SELECT` de `authenticated`: un `select` directo ya no los trae, solo `obras_ficha_persona()` (DEFINER, con contexto opcional). RLS SELECT ahora: `… AND (creado_por = auth.uid() OR obras_puede_ver_persona(id) OR obras_ctx_vigente('persona', id))` (`sql/092`; hasta ahí, `obras_persona_grant_ctx_vigente(id)`), y `obras_puede_ver_persona` = dueño / `obras_personas_todas` (sin ramas de vínculo ni referente; la rama de share directo se fue en `sql/086`). UPDATE exige `creado_por = auth.uid()`.
...
- **`obras_transferir`**: lo que el receptor ya recibía sobre lo que ahora es suyo se apaga primero (nadie se comparte consigo mismo; el CHECK `usuario_id <> otorgada_por` lo rechazaría). Lo compartido con terceros sigue vivo y ~~pasa a colgar del nuevo dueño (`otorgada_por`)~~ — `sql/093` borró esos tres UPDATE: la autoridad ya no está en la columna, así que la obra la revoca y la ve su responsable, que a partir de esa transferencia es el receptor. Los contactos del saliente vinculados a la obra y **no** tildados como exclusivos entran al receptor como grant contextual anclado a la obra (`sql/082`): los ve en la ficha, no en su agenda. Empresas sí van a la agenda completa — una empresa no es un contacto sensible.
...
- **Los vínculos que el saliente creó en obras de terceros** (`sql/051`) no se tocan **ni en el estado 3**: matarlos vaciaría la obra de alguien que no participó. Siguen vivos. ~~Y su `otorgada_por` pasa al receptor~~ — `sql/093` no reescribe más el otorgante: la autoridad sobre lo que cuelga de ellos se deriva del dueño del contacto y del ancla. Decisión del usuario: *"no lo saques, queda a decisión del nuevo dueño"*.
...
- **Lo que el saliente recibió pasa al entrante** (`usuario_id`), no solo lo que otorgó. Decisión del usuario: el reemplazo ocupa el lugar del que se fue, también para mirar. El tercero lo sigue viendo en su vista Compartido y puede revocarlo —es el dueño de lo compartido o del ancla (`sql/093`)—: ese es su recurso. Las colisiones se resuelven en tres pasos por tabla: se revive la fila que el entrante ya tenía para esa llave, se apaga la del saliente que no puede moverse (llave ocupada, u otorgante = entrante), y el resto cambia de mano. El EXISTS no filtra por `activo` porque el UNIQUE tampoco.
...
**`sql/097` — el dueño del contacto lo ve en la obra que le comparten.** A transfiere su obra a B dejando un contacto suyo ("No se va") y B después le comparte la obra: A la abría vacía, porque la rama de receptor de `obras_obra_persona_select` / `obras_obra_empresa_select` y de `obras_vinculos_de_obra` solo conocía "lo vinculé yo" y "me lo tildaron". Suma "el contacto es mío", dentro de la misma rama: dura lo que dure el compartir, y UPDATE no cambia. Test: `sql/tests/obras_097.sql`, 4/4.
...
`obras_persona_grant_contextual`: `(persona_id, usuario_id, obra_id XOR empresa_id, otorgada_por, activo)`, unique parcial por ancla. El contacto se ve solo desde esa ficha (`obras_ficha_persona(p_persona_id, 'obra'|'empresa', ctx_id)`); muere con el vínculo y con el ancla, validado en vivo por `obras_ctx_vigente('persona', id[, ancla])` (`sql/092`); desde `sql/099`, apagar el vínculo apaga además la fila (`cascada_desactivar`). Es **la única vía por la que un contacto llega a otro usuario** (`sql/082`, `sql/086`): la escriben el checklist de `obras_compartir_obra`, la cascada de `obras_transferir` y la rama persona de `obras_compartir_registros`. El ancla en NULL contesta "hay alguno vigente" y sirve a la policy de `obras_personas`; con ancla corta por padre, para que un grant traído por la obra A no abra la fila en la obra B. ~~Helpers anclados `obras_persona_grant_ctx_obra_conmigo` / `_empresa_conmigo`~~ — dropeados por `sql/092`, ver abajo.

`obras_empresa_grant_contextual` (`sql/085`): `(empresa_id, usuario_id, obra_id, otorgada_por, activo)`, UNIQUE entero por el trío — el ancla de una empresa solo puede ser una obra (una empresa no cuelga de otra empresa), así que no hay XOR ni índice parcial. La empresa se abre solo desde esa obra (`obras_ficha_empresa(empresa, ctx_obra)`); muere con el vínculo y con el ancla, validado en vivo por `obras_ctx_vigente('empresa', id[, 'obra', ancla])` (`sql/092`); desde `sql/099`, apagar el vínculo apaga además la fila. La escriben el checklist de `obras_compartir_obra`, la cascada de `obras_transferir` y la rama empresa de `obras_compartir_registros`. Mismo reparto que en persona: sin ancla sirve a la policy de `obras_empresas`, con ancla corta por padre. ~~Helper anclado `obras_empresa_grant_ctx_obra_conmigo`~~ — dropeado por `sql/092`. **Compartir una empresa desde su ficha no existe más** (`sql/086`): la única puerta es el checklist de la obra.

RLS: solo SELECT para `authenticated` (receptor, o quien tiene autoridad — `obras_es_mi_obra` / `obras_ctx_autoridad`, `sql/093`). La escritura pasa por `obras_compartir_obra` / `obras_revocar_obra` / `obras_revocar_contextual` / `obras_compartir_registros` / `obras_transferir*` (DEFINER, exigen `creado_por` / `responsable_id`).

**`sql/090` — el grant vale mientras se vea el ancla.** (Las cuatro funciones que nombra se unificaron en `obras_ctx_vigente` en `sql/092`.) `obras_persona_grant_ctx_vigente`, `obras_empresa_grant_ctx_vigente` y la rama contextual de `obras_ficha_persona` / `obras_ficha_empresa` piden además `obras_puede_ver_obra(ancla)` — o `obras_puede_ver_empresa(ancla) OR obras_empresa_grant_ctx_vigente(ancla)` para el ancla empresa. Antes solo verificaban que el vínculo entidad↔ancla siguiera vivo, así que un grant que quedaba activo por cualquier motivo seguía abriendo teléfono y email de alguien cuya obra ya no se ve. Consecuencias:
...
**`sql/091` — el receptor sin Obras, el array NULL y la cuarta copia del gate.** Lo que quedaba de la misma auditoría que se arregla en SQL sin decidir nada nuevo:

- **`obras_compartir_obra` exige `obras_ver` en el receptor** (`OB006`), junto al chequeo de `usuarios.activo`. Transferir ya lo hacía; compartir creaba un grant que no abre nada — la obra no aparece (`obras_select` exige `obras_ver`) y, desde `sql/090`, los contextuales que cuelgan tampoco. Se reusa `OB006` y no un código nuevo: es la misma regla, y un código por regla es justo lo que `OB022` dejó de cumplir. El texto sí cambia ("El usuario no tiene acceso a Obras"), porque acá no es el destino de una transferencia sino el receptor de un share, y `mensajeError()` muestra el texto de la base.
- **`COALESCE(p_*, '{}')` al entrar** en `obras_compartir_obra`, `obras_transferir` y `obras_transferir_empresa`. `id = ANY(NULL)` es NULL, no false, y el `DEFAULT '{}'` de la firma no aplica cuando el cliente manda `null` explícito. Compartir con `p_personas: null` no otorgaba **ni destildaba** —el `NOT (... = ANY(...))` de la limpieza también daba NULL, así que el reparto quedaba congelado— y `obras_transferir` con `p_migran: null` transfería la obra dejando al receptor sin ningún contacto, ni migrado ni contextual; además el chequeo `OB032` pasaba en silencio. Los schemas Zod ya tenían `.default([])` y el tipo rechaza `null`, así que esto blinda el PostgREST directo, no la UI. **No se tocó `obras_transferir_resolver_vinculos`**, que también recibe arrays: no tiene GRANT, solo la llaman las otras tres, ya saneadas.
- **`obras_migrar_agenda` pierde la cuarta copia del gate `OB006`** y pasa a `usuario_tiene_permiso(p_a_usuario, 'obras_ver')`, que incluye el `u.activo` del EXISTS que reemplaza. Sin cambio de conducta: cierra la serie que empezó `sql/090`.

Test: `sql/tests/obras_091.sql`, 5/5. **De yapa, cerró una deriva vieja**: las tres funciones que `sql/090` reescribió vivían en la base sin sus comentarios (la "versión despojada" que documenta la entrada cerrada del `BACKLOG.md`). Al aplicarse con el texto del repo, `md5(prosrc)` de las cuatro coincide ahora con el cuerpo del archivo.

**`sql/092` — la vigencia se escribe una sola vez.** Un grant contextual vale si (1) la fila de grant está activa, (2) el vínculo entidad↔ancla sigue vivo y (3) quien lo recibió sigue viendo el ancla. `sql/090` agregó (3) en cuatro lugares; la regla vivía en **siete**. Los tres `obras_*_grant_ctx_*_conmigo` no se tocaron y seguían chequeando solo (1).

**Y ahí había un agujero.** Dos de los tres se salvaban por el llamador —`obras_obra_persona_select` y `obras_obra_empresa_select` los usan detrás de `obras_obra_compartida_conmigo(obra_id)`, que es (3), y la fila que filtran es el vínculo mismo, que es (2)—. El tercero no tenía quién lo cubriera: `obras_persona_empresa_select` no verificaba el ancla en ningún lado, así que un grant de persona anclado en una empresa que el receptor ya no ve seguía dejando leer `cargo`, `es_principal` y `observaciones` de la fila persona↔empresa por PostgREST directo. Identidad, no contacto (revocado a nivel columna desde `sql/039` / `sql/085`), pero la misma clase que `sql/090` cerró.

- **`obras_ctx_vigente(p_tipo, p_entidad_id, p_ancla_tipo, p_ancla_id)`** (DEFINER STABLE, GRANT `authenticated`) — la regla, una sola vez. Ancla en NULL = "¿hay alguno vigente?"; ancla dada = "¿este?". Un `p_tipo` desconocido, o una empresa anclada en empresa, dan `false` sin `RAISE`.
- **Se dropean las cinco**: `obras_persona_grant_ctx_vigente`, `obras_empresa_grant_ctx_vigente`, `obras_persona_grant_ctx_obra_conmigo`, `obras_persona_grant_ctx_empresa_conmigo`, `obras_empresa_grant_ctx_obra_conmigo`. No quedan como envoltorios a propósito: sus nombres prometían (1) y nada más, y un nombre que dice menos que la regla es por donde volvió el bug.
- **Se reescriben** las cinco policies `_select` de `obras_personas` / `obras_empresas` / `obras_obra_persona` / `obras_obra_empresa` / `obras_persona_empresa`, la rama contextual de `obras_ficha_persona` / `obras_ficha_empresa` (colapsa a una línea) y las tres llamadas de `obras_vinculos_de_obra`.
- **Recursión**: la rama persona-anclada-en-empresa pregunta si se ve la empresa ancla, y una respuesta válida es que la empresa esté vigente por su propio grant → `obras_ctx_vigente` se llama a sí misma con tipo `empresa`. La cadena termina (la rama empresa nunca vuelve a persona) y Postgres acepta la autorreferencia en `LANGUAGE sql`.
- **Tres lecturas se endurecen**: la fila persona↔empresa del agujero; un vínculo obra↔persona / obra↔empresa **desactivado** deja de leerse por grant contextual (los callers ya filtran `activo`, así que no cambia ninguna pantalla); y en `obras_vinculos_de_obra` el `detalle` de una persona —la razón social de la empresa que representa— deja de salir cuando el grant de esa empresa quedó huérfano, coherente con que ese grant ya no abre su ficha desde `sql/090`.
- De paso, `auth.uid()` suelto pasa a `(select auth.uid())` en las dos policies que lo tenían así.

Test: `sql/tests/obras_092.sql`, 6/6. El caso B se verificó contra el predicado viejo en la misma transacción: daba `true` exactamente donde el nuevo da `false`.


**`sql/093` — `otorgada_por` es historia, no autoridad.** Cierra el segundo cambio estructural de la auditoría de `sql/090`. La autoridad sobre un grant contextual es **el dueño del contacto o el dueño del ancla**, y vive una sola vez en **`obras_ctx_autoridad(p_tipo, p_entidad_id, p_ancla_tipo, p_ancla_id)`** (DEFINER STABLE, GRANT `authenticated`) — hermana de `obras_ctx_vigente`: una contesta *quién manda*, la otra *si todavía vale*. No mira ninguna tabla de grant, así que puede usarse dentro de sus propias policies sin recursar (lo que `sql/045` no podía).

- **`obras_compartidos_por_mi`** deja de listar por `otorgada_por = auth.uid()`: la obra la lista su `responsable_id`, y los contextuales `obras_ctx_autoridad` **menos** las filas donde quien pregunta es el receptor (`usuario_id <> auth.uid()`) — una fila que me otorgaron no es algo que compartí. La vista lista exactamente lo que se puede revocar.
- **`obras_revocar_contextual`** cambia "dueño del ancla **o quien otorgó**" por la función, preguntada **antes** del UPDATE y no adentro del `WHERE`: cero filas pasa a ser idempotencia en vez de `OB026`, que era lo que recibía quien sí podía al reintentar sobre algo ya revocado. Mismo `OB031`, mismo `OB026`, texto nuevo.
- **Las tres policies `_select` de las tablas de grant** dicen ahora la misma regla: `obra_compartida_select` = receptor o `obras_es_mi_obra`; `grant_ctx_select` y `empresa_grant_ctx_select` = receptor o `obras_ctx_autoridad`. La de empresa (`sql/085`) ya derivaba del dueño de la entidad y le faltaba el ancla. **No era cosmético**: `getCompartidosObra` lee `obras_obra_compartida` directo, y sin esto el nuevo responsable de una obra transferida veía el panel "compartida con" vacío mientras el tercero seguía adentro.
- **Se van cinco UPDATE de `otorgada_por`**: tres en `obras_transferir`, uno en `obras_transferir_empresa` y el bloque 3.4 entero de `obras_migrar_agenda` (que además apagaba lo que 3.3 ya apaga, pero dicho por la columna equivocada; el viejo 3.5 pasa a ser 3.4). Existían para empujar la columna detrás de la propiedad cuando algo cambiaba de mano.
- **El checklist de `obras_compartir_obra`** apaga lo destildado filtrando por **dueño de la entidad** en vez de `otorgada_por`: la pantalla solo ofrece lo mío, así que solo puede apagar lo mío — un grant sobre el contacto de otro anclado a esta obra no aparece y ya no puede caerse por no estar tildado.
- **`otorgada_por` se sigue escribiendo** (siempre con el dueño de la entidad: lo exigen los cuatro escritores) y el **CHECK `usuario_id <> otorgada_por` se queda** — sigue siendo el guard de inserción "nadie se comparte consigo mismo", y el paso (b) de `obras_migrar_agenda` 3.4 lo necesita. Lo que cambia es que nadie la lee para decidir.

Test: `sql/tests/obras_093.sql`, 6/6. Deriva base↔repo al aplicarla: 0 filas con `otorgada_por` distinto del dueño, y `obras_revocar_contextual` —que vivía sin sus comentarios— quedó alineada.


**`sql/094` — las deudas chicas de la auditoría.** Tres inconsistencias sin acceso indebido detrás; ninguna toca autorización.

- **`OB009` vuelve a la vida y `OB022` significa una sola cosa.** `obras_ficha_persona` levanta `OB009` ("sin acceso a esta persona"), que es el código con el que nació en `sql/032` y que quedó muerto cuando `sql/039` la reescribió con `OB022`. No hizo falta un código nuevo, y la persona queda simétrica con la empresa, que ya tenía el suyo (`OB030`). `mensajeError()` no cambia: su lista blanca es el regex `^OB\d{3}$`. Cuatro tests seguían al viejo (`obras_090`, `obras_092`, `obras_compartir`) y uno —`obras_032` caso 09— llevaba desde `sql/039` esperando `OB009`: ese desfase se cierra solo.
- **`obras_ctx_vinculo_vivo(p_tipo, p_entidad_id, p_ancla_tipo, p_ancla_id)`** (DEFINER STABLE, **sin GRANT a `authenticated`**: solo la llaman dos DEFINER) — la mitad de la vigencia que no depende de quién pregunta: ¿sigue vivo el vínculo entidad↔ancla? Se extrajo de `obras_ctx_vigente`, que la usa en sus tres ramas sin cambiar de resultado.
- **La vista Compartido deja de listar grants que no abren nada.** El estado 3 de transferir (`p_sacar`, `sql/087`) desactiva los vínculos y deja `activo = true` en los grants contextuales de terceros que colgaban de ellos: el acceso moría bien desde `sql/090`, pero las filas seguían simulando un reparto. `obras_compartidos_por_mi` filtra ahora por `obras_ctx_vinculo_vivo` en sus dos ramas contextuales. No podía reusar `obras_ctx_vigente` entera: esa responde por `auth.uid()`, que acá es quien comparte y no quien recibe.
- **Columna nueva `puedo_abrir boolean`** en `obras_compartidos_por_mi` (el `RETURNS TABLE` cambia → `DROP` + `CREATE`; `ORDER BY 9` sigue apuntando a `compartida_el`). Resuelve lo que dejó `sql/093`: desde que la autoridad es "dueño del contacto **o** dueño del ancla", el dueño del ancla ve filas de contactos ajenos, y la UI linkeaba a una ficha que contesta `OB009`. `true` siempre en la rama obra (la lista su responsable); `obras_puede_ver_persona` / `_empresa` en las contextuales. `CompartidoView` no linkea cuando es `false` — el mismo techo que la ficha de la obra, que muestra el nombre del contacto ajeno y no el teléfono (`sql/070`, `sql/087`).

Tests: `obras_032` 12/12, `obras_086` 9/9, `obras_090` 7/7, `obras_092` 6/6, `obras_093` 6/6, `obras_compartir` 6/6.

**`sql/099` — sacar un contacto corta, desactivar la obra archiva.** Las dos preguntas abiertas de la segunda pasada de la auditoría.

- **El grant contextual muere con el vínculo, en la fila.** `obras_cascada_desactivar` suma tres ramas y cuelga de dos tablas más: apagar un `obras_obra_persona` apaga los `obras_persona_grant_contextual` anclados en ese par (obra, persona); apagar un `obras_obra_empresa`, los `obras_empresa_grant_contextual` de (obra, empresa); apagar un `obras_persona_empresa`, los de persona anclados en (empresa, persona). El unique parcial `WHERE activo` de cada puente garantiza que no queda otro vínculo vivo con el mismo par. Antes el grant quedaba `activo` y `obras_ctx_vinculo_vivo` lo dejaba dormido: volver a vincular lo despertaba sin que nadie tildara nada. `obras_ctx_vinculo_vivo` sigue dentro de `obras_ctx_vigente` como barrera. Limpieza al aplicar: 0 filas.
- **"Compartida conmigo" exige obra activa**, escrito una vez en `obras_obra_compartida_con` (JOIN a `obras`, `o.activo`). La rama de compartida de `obras_puede_ver_obra_de` pasa a llamarla en vez de repetir el EXISTS. Con eso el receptor de una obra desactivada pierde de una vez las policies de vínculos (`_conmigo`), la ficha (`obras_vinculos_de_obra` → `OB022`), los contextuales anclados (`obras_ctx_vigente` pasa por `obras_puede_ver_obra`) y el chip de tarea (`obras_puede_abrir`). Reactivar devuelve todo: ninguna fila de grant se tocó. **`obras_select` no cambia a propósito**: la fila es identidad, y es lo que resuelve el aviso y el nombre del chip bajo la RLS del receptor.
- **`obras_set_activo` no reactiva una obra rechazada** (`motivo_rechazo IS NOT NULL` → `OB035`). Rechazar deja `activo = false` y la función aceptaba volverla a `true`, salteando la autorización.
- **Aviso `obra_desactivada`**: trigger `trg_notificar_obra_desactivada` AFTER UPDATE OF activo ON `obras`, `WHEN (OLD.activo AND NOT NEW.activo)`, → `notificar_obra_desactivada()` (DEFINER, sin EXECUTE para nadie) le escribe a cada receptor con `obras_obra_compartida` activa. Corre también al rechazar un alta pendiente que ya estaba compartida. Ver `notificaciones.md`.
- **`obras_compartidos_por_mi`**: la rama obra y las contextuales ancladas en obra filtran `obras.activo`; `origen_nombre` sale solo si quien mira puede abrir el ancla (`obras_puede_ver_obra` / `obras_puede_ver_empresa`). Firma sin cambios. El nombre no se mostraba en pantalla, pero viajaba al navegador con el resto de la fila.

Test: `sql/tests/obras_099.sql`, 10/10. Revalidados: `rls_obras`, `eventos`, `acceso_registros`, `obras_036`, `051`, `086`, `087`, `088`, `090`, `092`, `093`, `095`, `096`, `097`, `obras_compartir`.
...
Checklist y vista: `obras_relaciones_compartibles_obra(obra, usuario)` → identidad mínima de lo mío vinculado a esa obra + `ya_compartida`. `obras_compartidos_por_mi()` (DEFINER, gate `obras_compartido`) → todo lo que puedo revocar, con quién y de qué origen, tope 500; por autoridad y no por `otorgada_por` desde `sql/093`, y solo con el vínculo vivo + `puedo_abrir` desde `sql/094`. Sin lo anclado en obras desactivadas y sin nombrar anclas que quien mira no puede abrir desde `sql/099`.
...
**Ver la fila no es abrir la obra (`sql/099`).** Para una obra desactivada, `obras_select` y `obras_puede_ver_obra_de` contestan distinto al receptor: la policy le deja la fila (identidad: nombre y datos generales) y la función le dice que no. La rama de compartida de la función pasa por `obras_obra_compartida_con`, que exige obra activa; la policy mantiene su EXISTS sin `activo`. Es la misma separación que `obras_personas`: la policy autoriza identidad y la ficha es otra puerta.
...
| `OB006` | `obras_transferir` · `_persona` · `_empresa` · `obras_migrar_agenda` · `obras_compartir_obra` (`sql/091`) | ese usuario no tiene acceso a Obras. Una regla, un código: en transferir/migrar el texto dice "el destino", en compartir "el usuario" |
...
| `OB035` | `obras_set_activo` (`sql/099`) | la obra fue rechazada: no se reactiva, hay que cargarla de nuevo |
...
Nunca DELETE. `obras_set_activo(uuid, boolean)` exige `obras_desactivar` **y** ser el responsable. Sin cascada sobre los vínculos ni sobre lo compartido: la obra se puede reactivar tal como estaba. Desactivada es archivada para todos (`sql/099`): el receptor la deja de abrir porque `obras_obra_compartida_con` exige obra activa, no porque se le apague la fila de grant, y le llega el aviso `obra_desactivada`. Una rechazada (`motivo_rechazo` con `activo = false`) no se reactiva: `OB035`.
...
Esa misma función cuelga además de `obras_obra_persona` (`sql/036`): desvincular a una persona de una obra baja su fila de `obras_obra_referente`. Sin eso el referente sobrevivía al vínculo —`getReferentes` lo lee sin pasar por él— y la comisión vieja reaparecía al volver a vincular a la misma persona. Las ramas van explícitas por `TG_TABLE_NAME`, y sigue `SECURITY DEFINER` porque la policy de UPDATE de `obras_obra_referente` exige `obras_referentes`, que quien desvincula puede no tener. Verificación: `sql/tests/obras_036.sql`, 8/8.

Desde `sql/099` cuelga también de `obras_obra_empresa` y `obras_persona_empresa`, y apagar cualquiera de los tres vínculos apaga los grants contextuales anclados en ese par. Las tablas de grant no tienen policy de UPDATE para `authenticated`: DEFINER hace falta también acá. Cinco tablas, cinco ramas.
