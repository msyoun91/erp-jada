...
Checklist y vista: `obras_relaciones_compartibles_obra(obra, usuario)` → identidad mínima de lo mío vinculado a esa obra + `ya_compartida`. `obras_compartidos_por_mi()` (DEFINER, gate `obras_compartido`) → todo lo que puedo revocar, con quién y de qué origen, tope 500; por autoridad y no por `otorgada_por` desde `sql/093`.
