...
- **Ni `obras_empresa_compartida` ni `obras_persona_compartida` tienen columnas de origen.** `origen_obra_id` se dropeó en `sql/085` (la de persona, en `sql/082`): los dos checklists escriben ahora una tabla de grant contextual, cuyo ancla **es** el origen. Las dos tablas quedan con un solo significado — acceso completo, acto directo del dueño, y `obras_persona_grant_directo` / `obras_empresa_grant_directo` colapsan a "¿hay grant activo?".
