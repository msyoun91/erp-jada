...
- `obras_empresa_compartida` gana `origen_obra_id` (nullable, `sql/047`): marca "este grant nació de tildar la empresa en el checklist de esa obra". Compartir por checklist setea el origen; compartir directo lo limpia (última escritura gana). `obras_revocar_obra` lo usa para la cascada. **`obras_persona_compartida` ya no tiene columnas de origen (`sql/082`)**: el checklist de persona escribe `obras_persona_grant_contextual`, cuyo ancla es el origen. Esta tabla tiene un solo significado — acceso completo, acto directo del dueño.
